...
extern INT8 Read_MSG_QUE(void); //interface with UI

extern int My_Strlen(const char * src);
extern void  Interference_Signal(Inter_Signal_St * st);
extern int kbhit(void);
extern void Read_Msg();

volatile unsigned long long int  send_counter = 0;  //send package num count
volatile unsigned long long int rev_counter = 0;
...
void Read_Msg()
{
    ...
    while(1)
    {
	 ...
	 if(0x46 == recv_data)  //0x46 = end
	 {
        	rev_counter++;
        	send_flag = 1;
	 }
     printf("Current successful Rev ack Package num:%lld\n", rev_counter);
    }
}
#define UP 65
#define DOWN 66
...
int main(int argc, char **argv)
{
     ...
     char str[100] = "";
    //char test[] = {66,66,66,66,66,66,66,66,66,65,65,65,65,65,65,65,65,65}; 
     //int i = 0;
    // Inter_Signal_St st;
     pthread_t send_pthid, rev_pthid, read_msg_pthid;
    ...
    err = pthread_create(&read_msg_pthid, NULL,(void *(*)(void *))Read_Msg, (void *)NULL);
     if(err != 0 )
        printf("Create Read_Msg pthread error!\n");
     else
#ifdef DEBUG
        printf("Create Read_Msg pthread success!\n");
#endif
     while(1)
     {
         /*if(!send_flag)
         {
            ...
            if(!send_flag)
            {
                Data_Protocal_Package(&package, PROPERTY_CMD, &temp, 1);
                Send_data(port_fd, &package,write);
                rev_counter--;
                send_flag = 0;
                usleep(300000);
                printf("Has packge lost resend successful Send package num:%lld\n", send_counter);
        ...
        /* while(!send_flag)
         {
            ...
            if(time_counter >= 20000000)
            {
                ...
                // tcflush(port_fd,TCIOFLUSH);
                 ...
                 send_counter--;
                 //usleep(1000);
                 printf("Rsesend successful Send package num:%lld\n", send_counter);
            }
	   ...
	   // if(18 == i) i = 0;
	    ...
	    // tcflush(port_fd,TCIOFLUSH);
         ...
         //time_counter = 0;
        // usleep(300000);
         printf("Current successful Send package num:%lld\n", send_counter);
        }
     }
}
